<template>
  <transition name="fade" mode="out-in">
    <div>
                ...
                <Form :model="data" label-position="top">
                    ...
                    <FormItem label="关联一篇文章">
                        <Select v-model="data.articleid" style="width:200px">
                            <Option v-for="item in msg" :value="item.sunwouId" :key="item.sunwouId">{{ item.title }}</Option>
                        </Select>
                    </FormItem>
                </Form>
                
                <!-- <div class="mtitle"><span style="color:red">*</span>模板名称</div>
                <Input v-model="data.modelName" size="large" placeholder="模板名称"/> -->
            </div>
        </div>
        <div>
            <div>模板截图</div>
        ...
        <div class="pubibtn" @click="submitFrom()">修改并发布模板</div>
                ...
                success(res){           
                    if(res.code){
                        ...
                        $.ajax({
                            ...
                            type: "POST",
                            data: {
                                query: JSON.stringify(that.pubQuery)
                            },
                            dataType: "json",
                            success(res){
                                if(res.code){
                                    if(res.params.result[0].articleid){
                                        that.data.articleid = res.params.result[0].articleid;
                                    }
                                    if(res.params.result[0].relation){
                                        that.data.relation = res.params.result[0].relation;
                                    }
                                    if(res.params.result[0].serverPrice){
                                        that.data.serverPrice = res.params.result[0].serverPrice;
                                    }
                                    that.data.modelType = res.params.result[0].modelType;
                                    that.data.modelCover = res.params.result[0].modelCover;
                                    that.data.modelName = res.params.result[0].modelName;
                                    that.data.funcDesc = res.params.result[0].funcDesc;
                                    that.data.status = res.params.result[0].status;
                                    that.data.sunwouId = res.params.result[0].sunwouId;
                                    that.data.modelPrice = res.params.result[0].modelPrice;
                                    that.data.industry = JSON.parse(res.params.result[0].industry);
                                    that.data.businssType = JSON.parse(res.params.result[0].businssType);
                                    that.data.modelImage = JSON.parse(res.params.result[0].modelImage);
                                    that.data.modelStyle = JSON.parse(res.params.result[0].modelStyle);
                                }else{
                                    that.$Message.error(res.msg)
                                }
                            }
