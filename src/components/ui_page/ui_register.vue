      ...
      <div class="height-all ui-login-panel">
        <div class="ui-login-header">
          ...
          <div class="panel-end">
            ...
            <div class="pointer font-white" style="margin-left:20px" @click="navTo('ui_login')">登录</div>
          </div>
        ...
        <div class="width-1200 margin-center panel-center sds">
          ...
          <div class="login-panel">
            <div class="title">注册Register</div>
            <div class="form-con">
            ...
            <div class="form-con">
              <Icon class="icon" type="ios-unlocked" size="18"></Icon>
              <input class="fm" v-model="data.password" type="password" placeholder="确认密码">
            </div>
            <div class="panel-between">
              <div class="form-con" style="width:150px">
                <Icon class="icon" type="android-radio-button-on" size="18"></Icon>
                <input class="fm" v-model="data.phoneCode" placeholder="验证码">
              </div>
              <div style="width:130px;overflow:hidden;margin-top:30px;height:40px">
                <div :class="istime ? 'sdsds panel-center item-center':'sdsds1 panel-center item-center'" @click="data.phone == '' ? ninPhone():''">{{istime ? timeNum:'获取验证码'}}</div>
                <div v-show="data.phone != ''" id="captchaBox" style="position:relative;top:-40px;opacity:0"></div>              
              </div>
            </div>
            
            <div class="login-btn" @click="regist()">立 即 注 册</div>
            <div class="panel-between" style="margin-top:20px">
              <div class="font-primary pointer fp-over" @click="navTo('ui_login')">返回登录</div>
              <div class="font-primary pointer fp-over"></div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ninPhone(){
      this.$Message.error("您还没填写手机号，咋获取啊？")
    },
    getYzm() {
      var that = this;
      $.ajax({
            ...
            initGeetest(
              ...
              function(captchaObj) {
                ...
                captchaObj.onSuccess(function(res) {
                  ...
                  $.ajax({
                    url: sessionStorage.getItem("API") + "user/secVali",
                    type: "post",
                    dataType: "json",
                    data: {
    ...
    regist() {
      ...
      if (this.data.phone == null) {
        ...
        $.ajax({
          url: sessionStorage.getItem("API") + "user/regist",
          type: "POST",
          data: this.data,
          dataType: "json",
          success(res) {
            if (res.code) {
              ...
              that.$router.push({ path: "/ui_login" });
            } else {
              that.$Message.error(res.msg);
            }
          }
